...
from sb3.environments.env_hybrid import HybridEnvTraining, HybridEnvEvaluation


def train_hybrid(env_name, total_timesteps, seed):
    # hyperparameters
    n_training_envs = 1
    n_eval_envs = 5

    # check environment
    #env = HybridEnvTraining()
    ...
    train_env = make_vec_env(HybridEnvTraining, n_envs=n_training_envs, seed=seed, monitor_dir=log_dir,
                             env_kwargs={'mode': 'training', 'name': env_name})
    eval_env = make_vec_env(HybridEnvEvaluation, n_envs=n_eval_envs, seed=seed,
                            env_kwargs={'mode': 'training', 'name': env_name})
