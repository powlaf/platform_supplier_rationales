...
from sb3.environments.env_centralized import CentralizedEnvTraining, CentralizedEnvEvaluation


def train_centralized(env_name, total_timesteps, seed):
    # hyperparameters
    n_training_envs = 1
    n_eval_envs = 5

    # check environment
    # env = CentralizedEnvTraining()
    ...
    train_env = make_vec_env(CentralizedEnvTraining, n_envs=n_training_envs, seed=seed, monitor_dir=log_dir,
                             env_kwargs={'mode': 'training', 'name': env_name})
    eval_env = make_vec_env(CentralizedEnvEvaluation, n_envs=n_eval_envs, seed=seed,
                            env_kwargs={'mode': 'validation', 'name': env_name})
    ...
    eval_callback = EvalCallback(eval_env, best_model_save_path=log_dir, log_path=log_dir,
                                 eval_freq=max(5_000 // n_training_envs, 1), n_eval_episodes=9999*5, deterministic=True,
                                  render=False)
