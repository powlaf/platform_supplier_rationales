...
from platform_pricing import optimize_hybrid
from simulations.classes import Customer, Supplier, Match


def hybrid_heuristic(patience, arrival_customers, arrival_suppliers, timesteps, commission, p_range, printl=True):
    ...
    for t in range(timesteps):
        ...
        pmin, pmax = optimize_hybrid(costs=costs, valuations=valuations, no_supplier=dist_suppliers,
                                     no_buyer=dist_customers, commission=commission, p_range=p_range)
        if printl:
            print(f'Price range: ({round(pmin,2)} - {round(pmax,2)})')

        # suppliers with costs above the price (- commission) are not participating this timestep
        active_suppliers = [s for s in suppliers if s.cost <= pmax * (1-commission)]
        active_customers = [c for c in customers if c.valuation >= pmin]

        totals['active_suppliers'][t] = len(active_suppliers)
        totals['total_suppliers'][t] = len(suppliers)
        totals['active_customers'][t] = len(active_customers)
        totals['total_customers'][t] = len(customers)

        if printl:
            print(f'Customer: {len(customers)} - Active ({len(active_customers)}) ({[round(c.valuation,2) for c in customers]})')
            print(f'Supplier: {len(suppliers)} - Active ({len(active_suppliers)}) (({[round(s.cost, 2) for s in suppliers]})')

        # uniform random matching
        matches = []
        short_side = 's' if len(active_customers) > len(active_suppliers) else 'c'
        primary, secondary = (active_suppliers, active_customers) if short_side == 's' else (active_customers, active_suppliers)
        ...
        for p in primary:
            ...
            # supplier sets own costs or minimum price as price
            price = sec.cost / (1 - commission) + 0.001 if short_side == 'c' else p.cost / (1 - commission) + 0.001
            price = pmin if price < pmin else price
...
def hybrid_drl(patience, arrival_customers, arrival_suppliers, timesteps, commission, p_range, printl=True, env_name='xxx'):
    ...
    for t in range(timesteps):
        ...
        if printl:
            print(
                f'Customer: {len(customers)} - Active ({len(active_customers)}) ({[round(c.valuation, 2) for c in customers]})')
            print(
                f'Supplier: {len(suppliers)} - Active ({len(active_suppliers)}) (({[round(s.cost, 2) for s in suppliers]})')

        # uniform random matching
        matches = []
        short_side = 's' if len(active_customers) > len(active_suppliers) else 'c'
        primary, secondary = (active_suppliers, active_customers) if short_side == 's' else (
        ...
        for p in primary:
            ...
            obervation = np.array([cost, cost / (1 - commission), pmin, pmax, patience - waiting_time, t]).astype(
                np.float32)
            price = model.predict(observation=obervation)[0].item()
