...
def decentralized_data_generation(patience, arrival_customers, arrival_suppliers, timesteps, commission, printl=True):

    # all C and S in the platform
    customers = []
    suppliers = []
    suppliers_trac = {'env_t': {t: [[], [], -1, -1] for t in range(timesteps)},
                      'env_g': [patience, arrival_customers, arrival_suppliers, commission],
                      'supplier': [],
                      'totals': {}
                      }
    # timesteps: [supplier_cost, customer_valuations, # suppliers, # customers]
    ...
    for t in range(timesteps):
        ...
        suppliers_trac['env_t'][t][0] = [s.cost for s in suppliers]
        suppliers_trac['env_t'][t][1] = [c.valuation for c in customers]
        suppliers_trac['env_t'][t][2] = len(suppliers)
