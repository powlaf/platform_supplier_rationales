...
def hybrid_data_generation(patience, arrival_customers, arrival_suppliers, timesteps, commission, p_range, printl=True):

    # all C and S in the platform
    customers = []
    suppliers = []
    suppliers_trac = {'env_t': {t: [[], [], -1, -1, -1.0, -1.0] for t in range(timesteps)},
                      'env_g': [patience, arrival_customers, arrival_suppliers, commission],
                      'supplier': [],
                      'totals': {}
                      }
    # timesteps: [supplier_cost, customer_valuations, # active suppliers, # active customers, pmin, pmax]
    ...
    for t in range(timesteps):
        ...
        suppliers_trac['env_t'][t][4] = pmin
        suppliers_trac['env_t'][t][5] = pmax
        ...
        suppliers_trac['env_t'][t][1] = [c.valuation for c in active_customers]
        suppliers_trac['env_t'][t][2] = len(active_suppliers)
        suppliers_trac['env_t'][t][3] = len(active_customers)
